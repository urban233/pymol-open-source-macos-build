...
class BuildMacOSExe:
  ...
  def __init__(self) -> None:
    ...
    self.build_script_filepath = pathlib.Path(
      PROJECT_ROOT_DIR / "pymol", "build_macos_exe.py"
    )
  ...
  def setup_build_environment(self) -> None:
    ...
    tmp_build_script_filepath = pathlib.Path(
      PROJECT_ROOT_DIR / "scripts/python", "build_macos_exe.py"
    )
...
def build_wheel() -> None:
  ...
  copy_pymol_sources()
  _CMD_FROM_BUILD_DIR = pathlib.Path(PROJECT_ROOT_DIR / "cmake-build-release" / "_cmd.cpython-311-darwin.so")
  _CMD_FROM_PRE_BUILT_DIR = pathlib.Path(PROJECT_ROOT_DIR / "pre-built" / "_cmd.cpython-311-darwin.so")
  if _CMD_FROM_BUILD_DIR.exists():
    shutil.copy(
      _CMD_FROM_BUILD_DIR,
      pathlib.Path(PROJECT_ROOT_DIR / "src/python/pymol" / "_cmd.cpython-311-darwin.so")
    )
  else:
    shutil.copy(
...
def main() -> None:
  ...
  build_macos_exe_parser = subparsers.add_parser('build-macos-exe', help="Builds the macOS EXE file.")
  build_macos_exe_parser.set_defaults(func=build_macos_exe)
